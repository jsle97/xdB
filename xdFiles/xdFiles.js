...
Description:
- xdFiles.js is a universal file and data management system for Node.js, with a focus on JSON file operations, but also supporting text and binary files.
- It provides a full range of CRUD operations for JSON files, including record management.
- It offers directory management features and file operations such as copying, moving, deleting, and streaming.
- Utilizes built-in Node.js modules: fs/promises, path, node:fs, crypto, zlib, stream, worker_threads, util.

Key functions and modules:
1.  Configuration (setConfig):
    -   Allows setting the base path, log level, cache parameters, encryption key, compression, versioning, and indexing settings.

2.  File Descriptors (FileDescriptor):
    -   Represent files, automatically analyzing their MIME type, checksum, statistics, and selecting the appropriate adapter.

3.  Adapters (Adapters - JSONAdapter, TextAdapter, BinaryAdapter, StreamAdapter):
    -   A modular architecture that allows for handling different file types.
    -   The ability to register custom adapters.
    -   JSON and Text adapters support indexing.

4.  File Operations:
    -   file.write, file.copy, file.delete, move.file: Basic file operations.
    -   file.stat, file.descriptor: Retrieving file metadata.
    -   file.chunk: Processing files in chunks.
    -   file.watch: Monitoring file changes.
    -   atomicWrite, atomicWriteBinary: Ensure atomic write operations by using temporary files.

5.  JSON Data Operations (modules `add`, `edit`, `del`, `view`):
    -   add.all, add.id: Adding data/records, with automatic ID generation and duplicate checking.
    -   edit.all, edit.id: Modifying data/records.
    -   del.all, del.id: Deleting data/records.
    -   view.all, view.id, view.more: Reading data with optional filtering, sorting, and pagination.

6.  Directory Management (dir.add, dir.del, dir.rename):
    -   Creating, deleting, and renaming directories.

7.  Streaming (stream, stream.pipe):
    -   Reading and writing files using Node.js streams.
    -   The ability to pipe data between files with optional transformation.

8.  Caching (Cache):
    -   getCacheKey, getCachedData, setCachedData, invalidateCache, clearAllCache: Managing optional in-memory cache for read data.

9.  Encryption and Compression:
    -   crypto.encrypt, crypto.decrypt: Encrypting and decrypting data (AES-256-CBC).
    -   compress.gzip, compress.gunzip: Compressing and decompressing data (Gzip).

10. Versioning (version.create, version.get, version.list):
    -   Optional creation and retrieval of previous file versions (primarily for JSON).

11. Indexing and Searching (index.file, index.search):
    -   Optional content indexing of files (JSON, text) for faster searching.

12. Queries (QueryBuilder - query()):
    -   Building complex queries for JSON data (filtering, field selection, sorting, limit, offset).

13. Batch Operations (Batch Operations - batch()):
    -   Executing multiple file operations as a single transaction with rollback capability in case of errors.

14. Event System (events.on, events.off, events.emit):
    -   Emitting and listening to events related to file operations (e.g., 'write', 'delete', 'copy').

15. Error Handling:
    -   createXdbError: Standardizing errors with predefined codes (XDB_ERROR_CODES) and messages.
    -   Logging operations at various levels (DEBUG, INFO, WARN, ERROR).

16. Utilities (utils):
    -   generateId: Generating unique identifiers.
    -   detectMimeType: Detecting MIME type based on extension or magic bytes.
    -   ensureDirectoryExists: Recursively creating directories.

Additional Features:
-   File Locks (fileLocks): Ensure data consistency during concurrent operations on the same file.
-   Asynchronous Operations: All I/O operations are asynchronous, ensuring non-blocking behavior.
-   No External Dependencies: The library relies solely on built-in Node.js modules.
*/
